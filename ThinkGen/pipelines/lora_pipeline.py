...
class ThinkGenLoraLoaderMixin(LoraBaseMixin):
    r"""
    Load LoRA layers into [`ThinkGenTransformer2DModel`]. Specific to [`ThinkGenPipeline`].
    """
    ...
    def lora_state_dict(
        ...
        **kwargs,
        ...
        if state_dict is None:
            raise ValueError("state_dict is None, please check the loading logic.")
        else:
            if isinstance(state_dict, tuple):
                state_dict = state_dict[0]
                if "base_model.model." in list(state_dict.keys())[0]:
                    state_dict = {k.replace("base_model.model.", "transformer."): v for k, v in state_dict.items()}
                print("print first 10 keys: \n", list(state_dict.keys())[:10])
